...
void get_local_spec(double **xill_spec, double *local_spec, double ctheta,
                    int nbins);
double trapz(double *ener, double *photar);

double Incl[] = {18.194874, 31.788328, 41.409622, 49.458397, 56.632984,
                 63.256313, 69.51268,  75.522484, 81.37307,  87.13402};
...
int main(int argc, char *argv[]) {
  ...
  // Input parameters: photons_data_filename, xill_spec_filename,
...
//  std::cout << ph_file << std::endl;
  ...
  for (int i = 0; i < xill_n_incl; i++) {
    for (int j = 0; j < xill_n_ener; j++) {
      // fscanf(xillver_data, "%lf %lf %lf\n", &xill_energy[j], &Emax_xill,
      // &xill_spec_new[j]);
      fscanf(xillver_data, "%lf %lf\n", &xill_energy[j], &xill_spec[i][j]);
    }
  }
  // xill_energy[xill_n_ener] = Emax_xill;
  xill_energy[xill_n_ener] =
      xill_energy[xill_n_ener - 1] +
      (xill_energy[xill_n_ener - 1] - xill_energy[xill_n_ener - 2]);

  fclose(xillver_data);

  /*------------End of reading xillver data file------------*/

  // double E_obs[number_of_bins+1], N_obs[number_of_bins],
  // local_numflux[number_of_bins];
  ...
  double *xill_spec_interpolated =
      (double *)malloc(xill_n_ener * sizeof(double));

  /*----------------- Reading photon data file and convolving with xillver
   * spectra ---------------*/
  ...
  while (!end_of_file) {
    ...
    rebin_spectrum(E_obs, local_spec, number_of_bins, xill_energy,
                   xill_spec_interpolated, xill_n_ener);
...
void rebin_spectrum(double *ener, double *flu, int nbins, double *ener0,
  ...
  // printf(" flu0 %d %lf %lf %lf\n", nbins-1, flu[nbins-1], flu[nbins-2],
  // flu[nbins-3]);
...
void get_local_spec(double **xill_spec, double *local_spec, double ctheta,
  ...
  for (int i = 0; i < nbins; i++)
    local_spec[i] = (xill_spec[ind][i] +
                     frac * (xill_spec[ind + 1][i] - xill_spec[ind][i]));
  // local_spec[i] = (xill_spec[ind][i] + frac * (xill_spec[ind + 1][i] -
  // xill_spec[ind][i])) * 0.5*cos(iobs);

  // return xill_spec[ind][i] + frac * (xill_spec[ind + 1][i] -
  // xill_spec[ind][i]);
}

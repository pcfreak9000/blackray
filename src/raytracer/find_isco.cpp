...
void find_isco(Real z1, Real &isco) {
  ...
  if (spin > 0.) rll = 1. + std::sqrt(1. - sqspin);
  else if (spin < 0.) rll = 1. + std::sqrt(1. - sqspin);
  else rll = 1.;
  ...
  if (a13 > -5.7) {
    ...
    omp = (-dmnp[0][3]
        + std::sqrt(dmnp[0][3] * dmnp[0][3] - dmnp[0][0] * dmnp[3][3]))
        / dmnp[3][3];
    omm = (-dmnm[0][3]
        + std::sqrt(dmnm[0][3] * dmnm[0][3] - dmnm[0][0] * dmnm[3][3]))
        / dmnm[3][3];
    ep = -(mnp[0][0] + mnp[0][3] * omp)
        / std::sqrt(-mnp[0][0] - 2. * mnp[0][3] * omp - mnp[3][3] * omp * omp);
    em = -(mnm[0][0] + mnm[0][3] * omm)
        / std::sqrt(-mnm[0][0] - 2. * mnm[0][3] * omm - mnm[3][3] * omm * omm);
    deold = 0.5 * (ep - em) / dr;
    ...
    do {
      ...
      ep = -(mnp[0][0] + mnp[0][3] * omp)
          / std::sqrt(
              -mnp[0][0] - 2. * mnp[0][3] * omp - mnp[3][3] * omp * omp);
      em = -(mnm[0][0] + mnm[0][3] * omm)
          / std::sqrt(
              -mnm[0][0] - 2. * mnm[0][3] * omm - mnm[3][3] * omm * omm);
      denew = 0.5 * (ep - em) / dr;
      ...
      if (std::fabs(denew) < std::fabs(detol)) {
        ...
        if (rnew < rold) rul = rnew;
        else if (rnew > rold) rll = rnew;
        else printf("rold=rnew? rold = %Le, rnew = %Le", rold, rnew);
      } else if ((denew * deold) < 0.0) {
        if (rnew < rold) rll = rnew;
        else if (rnew > rold) rul = rnew;
        else printf("rold=rnew? rold = %Le, rnew = %Le", rold, rnew);
      } else printf("Compare enew and eold. eold = %Le, enew = %Le\n", deold,
          denew);
